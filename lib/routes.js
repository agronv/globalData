...
export default class Routes {
  ...
  addPoint(startLat, startLon) {
    ...
    let pipes;
    if (this.value < 1) pipes = 1;
    else if (this.value < 2) pipes = 8;
    else pipes = 16; 

    const geometry = new THREE.TubeBufferGeometry(curve, 16, this.value, pipes, false);
...
// util function to convert lat/lng to 3D point on globe
  ...
  makeChart() {
    ...
    this.importBar.style = `height: ${(this.importSum / maxSum) * this.height * 0.75}px`;
    this.exportBar.style = `height: ${(this.exportSum / maxSum) * this.height * 0.75}px`;
