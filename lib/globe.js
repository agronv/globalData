...
document.addEventListener('DOMContentLoaded', () => {
    ...
    var camera, scene, renderer, width, height, mesh, geometry, loader, shader, uniforms, material, formerActive;
    ...
    function makeChart(exportSum, importSum) {
        ...
        importBar.style = `height: ${(importSum/maxSum)*600}px`;
        exportBar.style = `height: ${(exportSum/maxSum)*600}px`;
    ...
    function formatNum(num) {
        if (num >= 100000000) {
            return `$${parseFloat(num/1000000000, 10).toFixed(2)}B`;
        }
        else if (num >= 100000) {
            return `$${parseFloat(num / 1000000, 10).toFixed(2)}M`;
        }
        else if (num >= 100) {
            return `$${parseFloat(num / 1000, 10).toFixed(2)}K`;
        }
        return 0;
    }
