...
document.addEventListener('DOMContentLoaded', () => {
    ...
    Object.values(countries).forEach((country) => {
        ...
        countryLi.classList.add("invisible", "list-item");
    ...
    Object.keys(products).forEach((product) => {
        ...
        productLi.classList.add("invisible", "list-item");
    ...
    function importerExport(abbr, rawValue, imported) {
        ...
        if (imported) material = new THREE.LineBasicMaterial({ color: "rgb(45, 51, 227)" });
        else material = new THREE.LineBasicMaterial({ color: "rgb(215, 38, 38)" });
    ...
    function addPoint(startLat, startLon, value) {
        ...
        geometry = new THREE.TubeBufferGeometry(curve, 20, value, 20, false);
    ...
    function fetchCountry(e) {
        ...
        for (let i = 0; i < countriesList.children.length; i++) {
            countriesList.children[i].classList.add("invisible");
        }
        fetchData();
    }
    ...
    function fetchProduct(e) {
        ...
        for (let i = 0; i < productList.children.length; i++) {
            productList.children[i].classList.add("invisible");
        }
        fetchData();
    }
    ...
    function stringIncludeKey(string, key) {
        ...
        function _stringIncludeKey() {
            if (keyIdx === key.length) return true;
            if (stringIdx === string.length) return false;
    ...
    countryInput.addEventListener("input", (e) => {
        currentCountryName = e.target.value;
        for (let i = 0; i < countriesList.children.length; i++) {
            if (e.target.value !== "" && stringIncludeKey(countriesList.children[i].textContent, e.target.value)) {
                countriesList.children[i].classList.remove("invisible");
            }
            else countriesList.children[i].classList.add("invisible");
        }
    });
    ...
    productInput.addEventListener("input", (e) => {
        ...
        for (let i = 0; i < productList.children.length; i++) {
            if (e.target.value !== "" && stringIncludeKey(productList.children[i].textContent, e.target.value)) {
                productList.children[i].classList.remove("invisible");
            }
            else productList.children[i].classList.add("invisible");
        }
    });
