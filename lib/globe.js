...
document.addEventListener('DOMContentLoaded', () => {
    ...
    geometry = new THREE.SphereGeometry(175, 100, 100);
    ...
    function fetchData() {
        ...
        url = `https://atlas.media.mit.edu/hs92/import/2017/${currentCountry}/show/${currentProduct}/`;
    ...
    function importerExport(abbr, rawValue, imported) {
        ...
        if (imported) addPoint(parseFloat(nation.lat), parseFloat(nation.lon));
        else addPoint(parseFloat(nation.lat), parseFloat(nation.lon) + 2);
    }
    ...
    function addPoint(lat, lon) {
        ...
        point.position.x = 175 * Math.sin(phi) * Math.cos(theta);
        point.position.y = 175 * Math.cos(phi);
        point.position.z = 175 * Math.sin(phi) * Math.sin(theta);
