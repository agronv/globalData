...
export default class Arcs {
  constructor(scene, globeRadius, height, noData, shifter, globe) {
    this.scene = scene;
    this.height = height;
    this.noData = noData;
    this.minAltitude = 20;
  ...
  createArcs(importData, exportData, currentCountry) {
    ...
    if (importData.length || exportData.length) {
      this.globe.lookAtCountry(countries[this.currentCountry].lat, countries[this.currentCountry].long)
    }

    this.makeChart();
  }

  importOrExport(countryId, rawValue, isImported) {
    this.value = Math.pow(rawValue, 0.5) * 0.0003;
    if (this.value > 12) this.value = 12;
    if (this.value < 0.01) this.value = 0.01;

    if (isImported) this.material = new THREE.LineBasicMaterial({ color: "#18FBFF" });
