...
export default class Inputs {
  ...
  fetchData() {
    ...
    fetch(this.proxyUrl + url).then(response => response.json()).then(response => {
      this.loading.className = "";
      this.routes.createBars(response, this.currentCountry);
    });
  }
  ...
  generateProductSearch(key) {
    ...
    Object.values(products).forEach((product) => {
      productValue = this.stringKeyValue(product.name, key);
      if (productValue) {
        result.push({
          value: productValue,
          name: product.name,
  ...
  generateCountrySearch(key) {
    ...
    Object.values(countries).forEach((country) => {
      ...
      if (countryValue) {
        result.push({ value: countryValue, name: country.name, id: country.id });
      }
    });
    return this.quickSort(result);
  }
    ...
    if (this.countriesList.childElementCount && [40, 13, 38].includes(e.keyCode)) {
      if (e.keyCode === 40) {
        if (this.listIndex === null) {
          ...
          this.listIndex = this.listIndex + 1 % this.countriesList.childElementCount;
        }
        ...
        if (this.listIndex === null) {
          ...
          this.countriesList.children[this.listIndex].classList.remove("active-item");
          this.listIndex = this.listIndex - 1 || this.countriesList.childElementCount - 1;
        }
        this.countriesList.children[this.listIndex].classList.add("active-item");
        this.countryInput.value = this.countriesList.children[this.listIndex].innerText;
      }
    ...
    if (this.productList.childElementCount && [40, 13, 38].includes(e.keyCode)) {
      if (e.keyCode === 40) {
        if (this.listIndex === null) {
          ...
          this.listIndex = (this.listIndex + 1) % this.productList.childElementCount;
        }
        ...
        if (this.listIndex === null) {
          ...
          else this.listIndex = this.productList.childElementCount - 1;
        }
        this.productList.children[this.listIndex].classList.add("active-item");
        this.productInput.value = this.productList.children[this.listIndex].innerText;
      }
