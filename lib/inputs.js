...
export default class Inputs {
  constructor(scene, container, arcs, loading, noData) {
    ...
    // this.countryInput.addEventListener("keydown keyup", (e) => e.preventDefault());
    // this.productInput.addEventListener("keydown keyup", (e) => e.preventDefault());
  ...
  async fetchData(initialLoad = false) {
    this.loading.className = initialLoad ? "" : "loader";
  ...
  onKeyDown(e, list, input, editorType, callBack) {
    if (list.childElementCount && [40, 13, 38].includes(e.keyCode)) {
      if (e.keyCode === 40) {
        ...
        this.onEnter(editorType, list, callBack)
      }
  ...
  onEnter(editorType, list, callBack) {
    if (this.listIndex !== null) {
